...
├── skills/
│   ├── index.json              # Pre-built search index (generated by CI)
│   ├── east/
│   │   ├── SKILL.md
│   │   └── reference/
│   │       ├── api.md
...
const OUTPUT_FILE = './skills/index.json';
...
│                     BUILD TIME (CI)                              │
├─────────────────────────────────────────────────────────────────┤
│  reference/*.md  ──►  build-index.mjs  ──►  skills/index.json   │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
...
1. **SessionStart**: Only runs once per session, can take up to 10s
2. **UserPromptSubmit**: Runs on every prompt, should be <5s
   - Skip early if prompt doesn't contain East keywords
