...
async function searchAndFormat(query, filterSkills = null, limit = 5) {
  let miniSearch;
  try {
...
async function readRecentEntries(transcriptPath, maxEntries = 30) {
  let raw;
  try {
    const buf = await readFile3(transcriptPath);
    const slice = buf.length > MAX_READ_BYTES ? buf.subarray(buf.length - MAX_READ_BYTES) : buf;
...
function extractRecentContext(entries) {
  const texts = [];
  const files = [];
  let textCount = 0;
